...
The stored `type` is the notification's `notificationName(notification)` — an instance-level
`notificationType()` override when implemented, else the class-level `@Notification({ name })`,
else the class name. See [notificationType()](/docs/concepts/notifications#per-instance-display-name-notificationtype)
for when to reach for the instance override (e.g. one generic notification class carrying many
event names). This is also the value the [`?type=` inbox filter](/docs/recipes/in-app-notifications)
matches against.
...
| `getForNotifiable(notifiableType, notifiableId, types?)` | `Promise<StoredNotification[]>` | All notifications for a notifiable, newest first. Optional `types: string[]` restricts to matching `type` values. |
| `getUnread(notifiableType, notifiableId, types?)` | `Promise<StoredNotification[]>` | Unread notifications for a notifiable. Same optional `types` filter. |
| `delete(id)` | `Promise<void>` | Remove one notification. |
| `deleteOwned(id, owner)` | `Promise<boolean>` | Optional. Remove one notification only when it belongs to `owner` (`{ notifiableType, notifiableId, tenantId? }`); `true` when a row matched. |
| `markAsReadOwned(id, owner)` | `Promise<boolean>` | Optional. Mark one read only when it belongs to `owner`; `true` when a row matched, including an already-read one. |
| `findById(id)` | `Promise<StoredNotification \| null>` | Optional. Fetch one row — the ownership fallback when the two scoped mutations are absent. |

`types` is absent or an empty array by default, matching every type (unchanged behavior) — it's
purely additive. `PaginateForNotifiableOptions` carries the same `types?: string[]` for the
paginated read path. Every bundled adapter (in-memory, TypeORM, MikroORM, Prisma) implements the
filter; see [In-app notifications](/docs/recipes/in-app-notifications) for the higher-level
`NotificationsQueryService` and REST `?type=` query param this powers.
...
<Tabs items={['TypeORM', 'MikroORM', 'Prisma']}>
  ...
  <Tab value="MikroORM">
    ...
    <Callout type="info">
      Targets **MikroORM v7** (peers `^7`). It needs `@mikro-orm/decorators` because v7 moved the
...
To own the schema with your ORM's migrations instead, disable auto-create and use the exported
migration helpers:
...
<Tabs items={['TypeORM', 'MikroORM', 'Prisma']}>
  <Tab value="TypeORM">
    ...
    Call `createNotificationsTable(queryRunner)` inside a TypeORM migration. It's non-destructive
    (creates the table only if it doesn't exist) and driver-portable — the column types come from the
    entity metadata.

    ```ts title="migrations/1700000000000-AddNotifications.ts"
  ...
  <Tab value="MikroORM">
    ...
    ```ts title="migrations/AddNotifications.ts"
  ...
  <Tab value="Prisma">
    ...
    DDL. Add the [`Notification` model](#persistence-adapters) to your `schema.prisma` and apply it
    with `prisma migrate`:
...
<Callout type="info" title="Managed tables (schema-diff tooling)">
  The MikroORM and TypeORM adapters export `notificationsManagedTables()` — the table name(s) this
  store creates/manages (`['notifications']`), derived from the same set `ensureSchema()` scopes its
  fingerprint to. Feed it to your ORM's schema-diff exclude list (e.g. MikroORM's
  `schemaGenerator.skipTables`) so a consumer never hand-maintains this table name separately —
  mirroring `durableManagedTables()` / `telescopeManagedTables()` from the sibling `@dudousxd`
  ecosystem libs.

  ```ts
  import { notificationsManagedTables } from '@dudousxd/nestjs-notifications-database-mikro-orm';
