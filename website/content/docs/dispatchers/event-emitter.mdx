...
<Callout type="info">
  Already importing `EventEmitterModule.forRoot()` just for this? Pass `emitter: true` to
  `NotificationsModule.forRoot()` instead and drop the explicit import — see
  [Configuration → emitter](/docs/reference/configuration#emitter). Skip it if your app registers
  `EventEmitterModule.forRoot()` elsewhere already; registering it twice is unsafe.
</Callout>
