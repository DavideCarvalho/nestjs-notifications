...
- **Scheduled jobs survive restarts.** Delayed sends are stored in a Redis **sorted set**
  (`scheduledKey`, score = fire time); a poller moves due jobs onto the ready queue every
  `pollIntervalMs`. A process restart no longer loses a pending `setTimeout`.
- **Failed jobs retry, then dead-letter.** The worker delivers a job up to `maxAttempts` times
  (default `3`); a job that exhausts them is `LPUSH`ed to the **dead-letter queue** (`deadLetterKey`)
  instead of being dropped. Inspect or replay it from there.
