...
description: Pull a provider into a notification with @InjectService. The notification stays new-able with plain data, and the library fills the service from the Nest container at delivery time — sync and queued.
---
...
`@InjectService(Token)` solves this. Mark a property with it and the library resolves the provider
from the Nest container and assigns it **at delivery time** — before any channel runs. The
...
`new InvoicePaid(invoiceId)` carries the payload data. The `@InjectService` property is left
unset — there's no container in scope at the call site, and that's fine.

</Step>

<Step>

### The library injects before delivery

When the notification reaches a channel, the library walks its `@InjectService` properties and
resolves each token from the Nest container (`ModuleRef`), assigning it onto the instance. It runs
once per delivery and only fills properties that are still `undefined`, so it's cheap and safe to
repeat.
...
<Callout type="info">
  ...
  injection fills it lazily at delivery, an `@InjectService` property is normally `undefined` at
  serialize time and re-resolved fresh on the other side. Keep injected services out of `serialize()`.
</Callout>
...
<Callout type="warn">
  ...
  the same Nest application.
</Callout>

## See also

- [Notifications](/docs/concepts/notifications) — the decorator API and `@InjectService` in context
